...
data TransactionEnables
  = TransactionEnables_Spending
  | TransactionEnables_StakeForDelegating ModelAddress
  | TransactionEnables_PoolForDelegating
  | TransactionEnables_Withdrawing ModelAddress
  deriving (Eq, Ord, Show)
...
data TransactionState era = TransactionState
  ...
  , _transactionState_liveRewardAddresses :: Set ModelAddress
  , _transactionState_utxos :: Map Int {- action id -} (Map ModelUTxOId (ModelTxOut era))
  , _transactionState_txDependents :: Map {- action id -} Int (Map ModelTxId (NESet TransactionEnables))
  ...
  , _transactionState_withDepsInEpoch :: Map {- epoch -} Int (Map ModelTxId (ModelTx era, Map ModelTxId (NESet TransactionEnables)))
  }
...
newtype TransactionGraph (era :: FeatureSet) = TransactionGraph { unTransactionGraph :: FGL.Gr (Int {-, ModelTx era -}) (NESet TransactionEnables) }
...
freshRewardAddress :: MonadSupply Integer m => m ModelAddress
        ...
        case action of
          ...
          Action_Spend -> do
            ...
            let myOutputs = Map.union depOutputs inertOutputs
                Sum sumMyOutputs = flip foldMap myOutputs $ \(ModelTxOut _ (ModelValue mv)) -> case mv of
          ...
          Action_Withdraw -> do
            ...
            let Sum sumMyOutputs = flip foldMap depOutputs $ \(ModelTxOut _ (ModelValue mv)) -> case mv of
                  ModelValue_Inject (Coin v) -> Sum v
                  _ -> error "genTransactions: utxo has abstract value"
...
-- dependencies
addDependent :: MonadState (TransactionState era) m => Int -> ModelTxId -> [(TransactionEnables, Int)] -> m ()
